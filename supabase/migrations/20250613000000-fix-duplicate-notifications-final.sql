...
BEGIN
  -- حذف الإشعارات المكررة بناءً على معايير دقيقة
  WITH duplicates AS (
    SELECT id,
           ROW_NUMBER() OVER (
             PARTITION BY 
               ...
               book_submission_id,
               type,
               DATE_TRUNC('minute', created_at) -- نفس الدقيقة
...
-- تحديث التريغر لمنع الإشعارات المكررة نهائياً
  ...
  IF NEW.status = 'approved' AND OLD.status = 'pending' THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE user_id = NEW.user_id 
        ...
        AND type = 'success'
        AND created_at >= NOW() - INTERVAL '1 hour' -- في آخر ساعة
    ) THEN
