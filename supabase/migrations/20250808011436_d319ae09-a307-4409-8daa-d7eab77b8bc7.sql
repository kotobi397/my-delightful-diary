-- Ensure participant and sender/receiver avatar URLs are returned as public URLs
-- Update get_user_conversations to normalize avatar URLs
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE 'https://kydmyxsgyxeubhmqzrgo.supabase.co/storage/v1/object/public/avatars/' || p.avatar_url
...
-- Update get_conversation_messages to normalize sender/receiver avatar URLs
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN sender_p.avatar_url IS NULL OR sender_p.avatar_url = '' THEN NULL
      WHEN sender_p.avatar_url LIKE 'http%' THEN sender_p.avatar_url
      ELSE 'https://kydmyxsgyxeubhmqzrgo.supabase.co/storage/v1/object/public/avatars/' || sender_p.avatar_url
    ...
    CASE 
      WHEN receiver_p.avatar_url IS NULL OR receiver_p.avatar_url = '' THEN NULL
      WHEN receiver_p.avatar_url LIKE 'http%' THEN receiver_p.avatar_url
      ELSE 'https://kydmyxsgyxeubhmqzrgo.supabase.co/storage/v1/object/public/avatars/' || receiver_p.avatar_url
