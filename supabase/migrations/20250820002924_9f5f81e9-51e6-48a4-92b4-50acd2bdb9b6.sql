-- إلغاء الترحيل السابق وإنشاء نظام أكثر ذكاءً
-- يتعامل مع حالة الحسابات الإدارية التي ترفع كتب لمؤلفين مختلفين

-- حذف الـ trigger السابق
DROP TRIGGER IF EXISTS sync_author_bio_trigger ON public.profiles;
DROP FUNCTION IF EXISTS sync_author_bio_from_profiles();
DROP FUNCTION IF EXISTS migrate_author_bio_to_profiles();

-- إنشاء دالة ذكية للحصول على تعريف المؤلف
CREATE OR REPLACE FUNCTION get_author_bio(p_book_id uuid)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- إذا كان admin، ابحث في جدول authors أولاً
  IF v_is_admin THEN
...
-- إنشاء دالة محدثة للحصول على تفاصيل الكتاب مع التعريف الذكي
CREATE OR REPLACE FUNCTION get_book_details_with_smart_bio(p_book_id text)
RETURNS TABLE(
  id uuid,
  title text,
  subtitle text,
...
BEGIN
  ...
  SELECT 
    ...
    bs.author,
    bs.author_image_url,
    public.get_author_bio(bs.id) as author_bio,
    bs.category,
    bs.description,
    bs.language,
    bs.publication_year,
