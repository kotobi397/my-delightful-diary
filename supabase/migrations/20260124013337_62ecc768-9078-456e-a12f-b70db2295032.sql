-- Fix: correct column name + restrict usage to current auth user
CREATE OR REPLACE FUNCTION public.get_user_interaction_status(
...
BEGIN
  -- منع استخدام الدالة لمعرفة علاقات بين مستخدمين آخرين
  IF auth.uid() IS NULL OR auth.uid() <> p_current_user_id THEN
    RAISE EXCEPTION 'not allowed';
  END IF;

  -- التحقق من حالة المتابعة (اسم العمود الصحيح: following_id)
  SELECT EXISTS(
...
-- قفل التنفيذ ومنحه فقط للمستخدمين المسجلين
REVOKE ALL ON FUNCTION public.get_user_interaction_status(UUID, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_user_interaction_status(UUID, UUID) TO authenticated;