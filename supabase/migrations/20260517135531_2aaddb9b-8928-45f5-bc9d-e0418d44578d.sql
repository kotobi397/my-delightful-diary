...
BEGIN
  -- Fast path 1: exact UUID match (uses primary key)
  IF v_is_uuid THEN
    RETURN QUERY
  ...
  -- Fast path 2: exact slug match (uses idx_book_submissions_slug)
  RETURN QUERY
  SELECT bs.id, bs.user_id, bs.title, bs.subtitle, bs.author, bs.author_bio,
    bs.author_image_url, bs.category, bs.publisher, bs.translator,
    bs.description, bs.publication_year, bs.page_count, bs.language,
  ...
  -- Fast path 3: slug prefix (range scan on idx_book_submissions_slug)
  RETURN QUERY
  SELECT bs.id, bs.user_id, bs.title, bs.subtitle, bs.author, bs.author_bio,
    bs.author_image_url, bs.category, bs.publisher, bs.translator,
    bs.description, bs.publication_year, bs.page_count, bs.language,
  ...
  WHERE bs.status = 'approved'
    AND bs.slug >= p_book_id || '-'
    AND bs.slug <  p_book_id || '.'
  ORDER BY bs.created_at DESC
  LIMIT 1;
  IF FOUND THEN RETURN; END IF;

  -- Last resort: legacy fuzzy match (rare, full scan)
  RETURN QUERY
  SELECT bs.id, bs.user_id, bs.title, bs.subtitle, bs.author, bs.author_bio,
    bs.author_image_url, bs.category, bs.publisher, bs.translator,
    bs.description, bs.publication_year, bs.page_count, bs.language,
