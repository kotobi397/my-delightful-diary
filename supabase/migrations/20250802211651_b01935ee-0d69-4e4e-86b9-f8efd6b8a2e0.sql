...
AS $function$
DECLARE
  author_to_update text;
BEGIN
  -- في حالة INSERT مع حالة approved
  IF TG_OP = 'INSERT' AND NEW.status = 'approved' THEN
    author_to_update := NEW.author;
  END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.status != NEW.status THEN
      ...
      -- تحديث المؤلف القديم إذا تغير اسم المؤلف
      IF OLD.author != NEW.author THEN
        UPDATE public.authors 
        SET books_count = (
          SELECT COUNT(*)
          FROM public.book_submissions bs
          WHERE bs.author = OLD.author 
          AND bs.status = 'approved'
        )
  ...
  IF TG_OP = 'DELETE' AND OLD.status = 'approved' THEN
    author_to_update := OLD.author;
  END IF;
  
  -- تحديث عدد الكتب للمؤلف المتأثر
  IF author_to_update IS NOT NULL THEN
    UPDATE public.authors 
    SET books_count = (
      SELECT COUNT(*)
      FROM public.book_submissions bs
      WHERE bs.author = author_to_update 
      AND bs.status = 'approved'
    )
    WHERE name = author_to_update;
  END IF;
