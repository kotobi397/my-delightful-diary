...
-- إصلاح الفهارس لتحسين الأداء
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_book_submissions_status_approved 
ON book_submissions(status) WHERE status = 'approved';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_book_reviews_book_id 
ON book_reviews(book_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_navigation_history_session_path 
ON navigation_history(session_id, path);
