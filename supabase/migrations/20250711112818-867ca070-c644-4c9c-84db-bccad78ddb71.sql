-- تحديث دالة get_book_details لدعم البحث بالـ slug
...
BEGIN
  ...
  -- إذا لم توجد نتائج بالـ slug، جرب البحث بالعنوان والمؤلف كـ fallback
  IF NOT FOUND THEN
    -- فصل العنوان والمؤلف من الـ slug
    DECLARE
      slug_parts text[];
      reconstructed_slug text;
    BEGIN
      slug_parts := string_to_array(replace(p_book_id, '-', ' '), ' ');
      
      -- البحث في الكتب المعتمدة بمطابقة تقريبية
      RETURN QUERY
      SELECT 
        bs.id::text,
        bs.title,
      ...
      WHERE bs.status = 'approved'
        AND (
          LOWER(replace(bs.title || ' ' || bs.author, ' ', '-')) = LOWER(p_book_id)
          OR 
          LOWER(replace(bs.title || '-' || bs.author, ' ', '-')) = LOWER(p_book_id)
        )
      LIMIT 1;
