...
-- Clean existing conflicting objects
...
-- Ensure consistent admin check function
...
-- Rebuild admin_users policies without recursion
...
-- Challenges policies: restrict admin checks to non-SELECT
...
-- Drop old score function then recreate
DROP FUNCTION IF EXISTS public.update_challenge_score(uuid, uuid, text, integer, jsonb);

CREATE FUNCTION public.update_challenge_score(
  p_challenge_id uuid,
  p_user_id uuid,
